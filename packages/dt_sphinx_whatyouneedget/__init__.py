...
class RequirementsDirective(SphinxDirective):
    ...
    def run(self) -> List[nodes.Node]:
        ...
        if SEPARATOR not in lines:
            raise ValueError("Requirements directive needs a separator '---' between what you will need "
                             "and what you will get")

        if lines.count(SEPARATOR) != 1:
            raise ValueError("Requirements directive needs exactly one separator '---' between what you will need "
                             "and what you will get")
